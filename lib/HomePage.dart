                    ...
                    FutureBuilder(
                      ...
                      builder: (BuildContext context,
                        ...
                        if (snapshot.connectionState == ConnectionState.done &&
                            snapshot.hasData) {
                          return Column(
                            children: [ Image.network(snapshot.data.iconUrl),
                              Text("${((((snapshot.data.tempInfo.temprature) - 32)*5)/9)
                                  .toInt()}°", style: TextStyle(fontSize: 40,color: Colors.white)),
                              Text(snapshot.data.weatherInfo.description, style: TextStyle(fontSize: 30,color: Colors.white)),SizedBox(height: 150,),
                          if(((((snapshot.data.tempInfo.temprature) - 32)*5)/9)>30)
                            Text('its time for 🍦 in ${widget.city} !', style: TextStyle(fontSize: 30,color: Colors.white)),
                              if(((((snapshot.data.tempInfo.temprature) - 32)*5)/9)<30)
                                Text('its time for ☕ in ${widget.city} !', style: TextStyle(fontSize: 30,color: Colors.white))
              ...
              if(_response != null && tempchange ==true )
                Column(
                  children: [
                    ...
                    Text("${((_response.tempInfo.temprature)-273).toInt()}°" , style: TextStyle(fontSize: 40,color: Colors.white),),
                    Text(_response.weatherInfo.description, style: TextStyle(fontSize: 30,color: Colors.white)),SizedBox(height: 150,),
                    if(_response.tempInfo.temprature-273>30)
                      Text('its time for 🍦 in ${_response.cityName} !', style: TextStyle(fontSize: 30,color: Colors.white)),
                    if(_response.tempInfo.temprature-273<30)
                      Text('its time for ☕ in ${_response.cityName} !', style: TextStyle(fontSize: 30,color: Colors.white))

                  ],
                ),
              if(_response != null && tempchange ==false )
                Column(
                  children: [
                    Image.network(_response.iconUrl),
                    Text("${((((_response.tempInfo.temprature)-32)*5)/9).toInt()}°" , style: TextStyle(fontSize: 40,color: Colors.white),),
